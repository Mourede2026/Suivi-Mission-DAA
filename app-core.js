...
function onLoginResult(res, alertId) {
  ...
  ['missions', 'vehicules', 'mon-suivi', 'activites', 'finances', 'comptes'].forEach(p => {
...
function loadFinances() {
  ...
  api('getRealisationFinanciere', [SESSION.token, STATE.annee]).then(renderFinances).catch(onSessionError);
...
document.getElementById('btn-add-user').addEventListener('click', () => {
  ...
  api('addUser', [SESSION.token, data]).then(res => {
    ...
    ['u-nom','u-id','u-pw','u-qualite','u-observations'].forEach(id => document.getElementById(id).value = '');
...
function renderUsers(users) {
  document.getElementById('users-table-wrap').innerHTML = `<table><thead><tr><th>Nom</th><th>Identifiant</th><th>Rôle</th><th>Actif</th><th></th></tr></thead><tbody>
    ${users.map(u => `<tr><td>${escapeHtml(u.nom)}</td><td>${escapeHtml(u.identifiant)}</td><td>${escapeHtml(u.role)}</td>
