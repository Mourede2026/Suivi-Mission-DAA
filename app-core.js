...
function onLoginResult(res, alertId) {
  ...
  document.getElementById('login-modal-overlay').style.display = 'none';
  document.getElementById('login-id-global').value = '';
  document.getElementById('login-pw-global').value = '';
...
function drawDashboardComptable() {
  ...
  statsWrap.innerHTML = `
    <div class="kpi-row">
      <div class="kpi"><div class="n">${g.executee}</div><div class="l">Exécutées (zone)</div></div>
      <div class="kpi"><div class="n">${g.en_cours}</div><div class="l">En cours (zone)</div></div>
      <div class="kpi"><div class="n">${g.non_realisee}</div><div class="l">Non réalisées / à venir (zone)</div></div>
      <div class="kpi"><div class="n">${g.en_retard}</div><div class="l">En retard (zone)</div></div>
    </div>
    <table><thead><tr><th>Responsable</th><th>Exécutées</th><th>En cours</th><th>Non réalisées</th><th>En retard</th><th>Total</th></tr></thead><tbody>
      ${lignes.length === 0 ? '<tr><td colspan="6" class="empty">Aucune mission.</td></tr>' : lignes.map(s => `<tr><td>${escapeHtml(s.responsable)}</td><td>${s.executee}</td><td>${s.en_cours}</td><td>${s.non_realisee}</td><td>${s.en_retard}</td><td><b>${s.total}</b></td></tr>`).join('')}
    </tbody></table>`;
  ...
  missionsWrap.innerHTML = missionsFiltrees.length === 0 ? '<div class="empty">Aucune mission.</div>' : `<table><thead><tr><th>ID</th><th class="col-libelle">Intitulé</th><th>Responsable</th><th>Exécution</th></tr></thead><tbody>
    ${missionsFiltrees.map(m => {
      const labels = { executee: ['Exécutée', 'badge-ok'], en_cours: ['En cours', 'badge-occupe'], non_realisee: ['Non réalisée', 'badge-nonplanifiee'], en_retard: ['En retard', 'badge-warn'] };
